...
from transform import data_clean as dc
...
def write_sqlite(db_path: str, include_raw: bool = False):
    ...
    try:
        ...
        teacher_tag_counts_out = teacher_tag_counts[["teacher_id", "tag", "n"]].copy()
        teacher_course_tag_counts_out = teacher_course_tag_counts[
            ["teacher_id", "class_code", "tag", "n"]
        ].copy()
        ...
        _df_to_sql(conn, "teacher_course_tag_counts", teacher_course_tag_counts_out)
        ...
        for t in [
            ...
            "teacher_tag_counts",
            "teacher_course_tag_counts",
        ] + (["ratings_raw"] if include_raw else []):
...
def main():
    p = argparse.ArgumentParser(description="convert dataframes into sqlite3 database")
    p.add_argument("--db", required=True, help="path to sqlite file (ex: rmp_snapshot.sqlite)")
    p.add_argument(
        "--include-raw",
        action="store_true",
